                ...
                for tool in tool_calls:
                    ...
                    if typed_tool["type"] == "function":
                        ...
                        if "arguments" in typed_tool["function"]:
                            raw_arguments = typed_tool["function"]["arguments"]
                            try:
                                arguments = json.loads(raw_arguments)
                            except json.JSONDecodeError:
                                # Small open-weight models occasionally emit
                                # trailing garbage after a complete JSON value
                                # in a tool call's arguments string (e.g. a
                                # repeated/echoed object). That tool_call is
                                # then persisted in the conversation history,
                                # so a bare json.loads() here doesn't just
                                # fail once -- it fails on *every* subsequent
                                # turn/retry that resends this same history,
                                # permanently wedging the session (neither
                                # Codex's own reconnect retries nor Zero's
                                # resume --last retries can recover, since
                                # they all resend the same poisoned message).
                                # Recover by decoding only the first complete
                                # JSON value and discarding trailing data,
                                # falling back to {} if even that fails.
                                try:
                                    arguments, _ = json.JSONDecoder().raw_decode(raw_arguments)
